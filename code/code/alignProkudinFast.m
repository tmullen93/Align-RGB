...
% List of images
imageNames = {'melons.tif'};%,'00125v.jpg',	'00153v.jpg', '00398v.jpg', '00149v.jpg', '00351v.jpg',	'01112v.jpg'};
...
for i = 1:length(imageNames),
    ...
    shrinkShift=20*shrinkAlign(channels,maxShift,.05);
    
    final=channels(:,:,1);
    final(:,:,2)=circshift(channels(:,:,2),shrinkShift(1,:));
    final(:,:,3)=circshift(channels(:,:,3),shrinkShift(2,:));

    newChannels=final;


    % Align the blue and red channels to the green channel
    [colorIm, predShift] = alignChannels(newChannels, [5 5]);
    
    % Print the alignments
    fprintf('%2i %s shift: B (%2i,%2i) R (%2i,%2i)\n', i, imageNames{i}, predShift(1,:)+shrinkShift(1,:), predShift(2,:)+shrinkShift(2,:));
